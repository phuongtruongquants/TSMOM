...
from tsmom.visibility import REGIME_COLORS, compute_risk_scale, detect_regimes, rolling_graph_features, scan_stock, scan_universe
...
with tab_graph:
    ...
    **Y tuong:** Chuyen doi chuoi gia co phieu thanh mot mang luoi (network), sau do trich xuat
    ...
    # ── Stock Scanner ──────────────────────────────────────
    st.markdown("---")
    st.subheader("Stock Scanner — quet toan bo danh muc")

    st.markdown("""
    Bo scanner chay visibility graph + Louvain cho **toan bo 60 co phieu**.
    Moi co phieu duoc phan loai theo:

    - **Regime**: so nhom regime hien tai (cang nhieu = cang phan manh)
    - **Xu huong regime**: dang mo rong hay thu hep (expanding/contracting)
    - **Muc rui ro (Risk Scale)**: 1.0 = binh thuong, < 0.95 = canh bao, < 0.5 = nguy hiem
    - **Diameter + Betweenness Variance**: chi bao cau truc mang

    Bang duoc sap xep tu **rui ro cao nhat -> thap nhat**.
    """)

    scanner_lookback = st.slider("Lookback (ngay)", 30, 120, 60, key="scanner_window")

    if st.button("Quet toan bo danh muc", key="run_scanner"):
        with st.spinner(f"Dang quet {len(daily_prices.columns)} co phieu..."):
            try:
                scanner_report = scan_universe(daily_prices, lookback_days=scanner_lookback)

                if scanner_report.empty:
                    st.warning("Khong co du lieu scanner.")
                else:
                    col_a, col_b, col_c, col_d = st.columns(4)
                    col_a.metric("Binh thuong", f"{(scanner_report['risk_level'] == 'normal').sum()}")
                    col_b.metric("Canh bao", f"{(scanner_report['risk_level'] == 'warning').sum()}")
                    col_c.metric("Nguy hiem", f"{(scanner_report['risk_level'] == 'danger').sum()}")
                    col_d.metric("Risk trung binh", f"{scanner_report['risk_scale'].mean():.3f}")
                    ...
                    styled = scanner_report.style.applymap(_color_risk, subset=["risk_level"]).format({
                        "risk_scale": "{:.3f}",
                        "diameter": "{:.1f}",
                        "betw_var": "{:.6f}",
                    })
                    ...
                    risky = scanner_report[scanner_report["risk_level"] != "normal"]
                    if not risky.empty:
                        st.warning(
                            "**" + str(len(risky)) + " co phieu can chu y:** "
                            + ", ".join("**" + str(r["symbol"]) + "**" for _, r in risky.iterrows())
                        )

                    # Regime map detail
                    st.markdown("---")
                    st.subheader("Regime Map — xem chi tiet 1 co phieu")
                    scan_symbol = st.selectbox(
                        "Chon co phieu",
                    ...
                    if st.button("Xem Regime Map", key="run_regime_detail"):
                        with st.spinner(f"Dang ve regime map cho {scan_symbol}..."):
                            prices = daily_prices[scan_symbol].dropna()
                            communities, node_colors = detect_regimes(prices)

                            fig, ax = plt.subplots(figsize=(14, 5), dpi=plot_dpi)
                            for i in range(len(prices) - 1):
                                ax.plot(prices.index[i:i + 2], prices.values[i:i + 2],
                                       color=node_colors[i], lw=1.5, alpha=0.8)
                            ax.set_title(f"Regime Map — {scan_symbol} ({len(communities)} regimes)")
                            ax.set_ylabel("Price")
                            ...
                            for cid in range(min(len(communities), 10)):
                                legend_elements.append(Patch(
                                    facecolor=REGIME_COLORS[cid % len(REGIME_COLORS)],
                                    label=f"R{cid + 1} ({len(communities[cid])}d)",
                                ))
                            ...
                            detail = scan_stock(prices, lookback_days=scanner_lookback)
                            if "error" not in detail:
                                st.markdown(
                                    f"**{scan_symbol}**: Regime `{detail['regime_label']}` "
                                    f"({detail['regime_trend']}), "
                                    f"{detail['latest_regime_days']}d in current regime. "
                                    f"Risk: **{detail['risk_level']}** ({detail['risk_scale']:.3f}). "
                                    f"Diameter: {detail['diameter']:.1f}, Betw_var: {detail['betw_var']:.6f}."
                                )
